...
_Last run: 2026-07-04 (steward). Overwritten each run; latest state only._
...
- **[Draft PR #13](https://github.com/j0hnegan/today/pull/13) "Native iOS app shell (Capacitor) [WIP]"**
  (`feat/ios-capacitor`) — **34 days open** (since 2026-05-31). No backlog folder. 30-day flag triggered 2026-06-30 — decision still pending. Three options:
...
- **`feature/task-triage-v2`** — local branch, 13 days old (1 commit 2026-06-21), no PR filed, no backlog folder. Options: file a PR, create a backlog folder via `/backlog`, or merge/close if it was a one-off.

- **`origin/claude/fix-task-categorization-j8nuy`** — old bug-fix branch (~64 days, 2026-05-01), 1 commit ahead of main, no PR ever filed. Check whether `feature/task-triage-v2` addresses this; if so, close the branch.

- **019 Plaid→Chase sync stays blocked** — needs 4-table migration + Plaid account + env vars. 018 Finance tab queued right behind it.

- **011 search** — `proposed`, awaiting your one-tap go/no-go (27 days since intake). Reply "go on 011" to build Phase 1 (docs/notes search).
...
- Active features last touched: 011 (27 days), 018 (23 days), 019 (23 days) — none >30 days.
