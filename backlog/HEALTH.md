...
_Last run: 2026-07-09 #3 (steward, daily health). Overwritten each run; latest state only._
...
- Nothing to fix. Re-audited LEARNINGS.md (no dead refs, no dupes, no contradictions —
  ...
  2026-07-08 #2 steward run — two quiet builder runs landed in between (dashboard/
...
- **011 search — 32 days untouched** (intake 2026-06-07, still only the original
...
- **[Draft PR #13](https://github.com/j0hnegan/today/pull/13) "Native iOS app shell (Capacitor) [WIP]"**
  (`feat/ios-capacitor`) — **39 days open** (since 2026-05-31). No backlog folder. Three options:
...
- **`feature/task-triage-v2`** — local branch, 18 days old (1 commit 2026-06-21), no PR filed, no backlog folder. Options: file a PR, create a backlog folder via `/backlog`, or merge/close if it was a one-off.

- **`origin/claude/fix-task-categorization-j8nuy`** — old bug-fix branch (~69 days, 2026-05-01), 1 commit ahead of main, no PR ever filed. Check whether `feature/task-triage-v2` addresses this; if so, close the branch.
...
- Active features last touched: 011 (32 days — flagged above), 018 (28 days), 019 (28 days).
