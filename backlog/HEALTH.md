...
_Last run: 2026-07-22 (steward). Overwritten each run; latest state only._
...
## Fixed this run
- Nothing to fix. Backlog structure and status fields are unchanged since the
  builder's quiet run (`184daaa`, 2026-07-21). Re-verified: all 19 folders still
  have `spec.md` + `log.md`, all `Status:` values still valid (16 shipped, 011
  proposed, 018 ready, 019 blocked), and all agree with `gh pr list` (#13 remains
  the only open PR — no new PRs, no state changes). LEARNINGS.md re-checked —
  still no dead refs (`lib/server-fetchers.ts` and `lib/validation/` confirmed
  present), no dupes, no contradictions; unchanged since 2026-06-10. No
  `backlog/PAUSED` kill switch. Refreshed the dashboard "Last run" line.

## Needs John

### Standing (unchanged from prior runs)

- **011 search — 45 days untouched** (intake 2026-06-07, still only the original
...
- **018 and 019 — 41 days since last `log.md` activity** (2026-06-11). Not neglect —
...
- **[Draft PR #13](https://github.com/j0hnegan/today/pull/13) "Native iOS app shell (Capacitor) [WIP]"**
  (`feat/ios-capacitor`) — **52 days open** (since 2026-05-31). No backlog folder. Three options:
...
- **`feature/task-triage-v2`** — local branch, 1 commit (2026-06-21, 31 days), no PR filed, no backlog folder. Options: file a PR, create a backlog folder via `/backlog`, or merge/close if it was a one-off.

- **`origin/claude/fix-task-categorization-j8nuy`** — old bug-fix branch (~82 days, 2026-05-01), 1 commit ahead of main, no PR ever filed. Previously diffed against `feature/task-triage-v2`: the latter is a much larger rewrite (163 files touched) that includes its own task-categorization changes plus migrations, config, and tooling churn — not a clean superset, so can't confirm it fully subsumes the older branch without a closer read. Worth a look before closing either.
...
- Active features last touched: 011 (45 days — flagged above), 018 (41 days), 019 (41 days).
